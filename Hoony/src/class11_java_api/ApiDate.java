...
public class ApiDate {
	public static void main(String[] args) throws ParseException {
		...
		// Date 객체가 생성되는 순간 해당 date 객체 내부에 그 때의 시간이 세팅된다.
...
//		System.out.println(newDate);
		...
		strToday = dateFormat.format(1647493925941L); // L은 long타입
		System.out.println(strToday);
		...
		// 문자열에 포함되어 있지 않은 연, 월, 일은 기본값인 1970년 1월 1일로 되어 있다.
		
		String strYear = "2022.03.17";
		sdf = new SimpleDateFormat("yyyy.MM.dd");
		Date dateYear = sdf.parse(strYear);
		System.out.println(dateYear);
		// 문자열에 포함되어 있지 않은 시, 분, 초 밀리초는 기본값인 0으로 되어 있다.
		...
		// Calendar 사용
		// 한 날짜를 기준으로 날짜 혹은 시간을 더하거나 뺄 때 유용
		...
		// Calendar의 getInstance()는 싱글톤이 아니라 새로운 Calendar 객체를 생성하는 것이다.
		Calendar calTest = Calendar.getInstance();
		// new Date() 처럼 실행될 당시의 시간이 세팅된 캘린더 객체가 생성된다
		...
		System.out.println(calTest2.getTimeInMillis());
		...
		// 해당 월의 마지막 일(DATE)을 리턴한다
		int lastDay = calendar.getActualMaximum(Calendar.DAY_OF_MONTH);
		System.out.println(lastDay);
		// 해당 월의 시작 요일(1: 일요일, 2: 월요일 ... 7: 토요일)
